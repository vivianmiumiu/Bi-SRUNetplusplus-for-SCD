...
class NestedUNet(nn.Module):
    def __init__(self, args,in_channel,out_channel):
        ...
        self.final1 = nn.Conv2d(nb_filter[3], out_channel, kernel_size=1)
        self.final2 = nn.Conv2d(nb_filter[2], out_channel, kernel_size=1)
        self.final3 = nn.Conv2d(nb_filter[1], out_channel, kernel_size=1)
    ...
    def forward(self, input):
        ...
        output1 = self.final1(x3_1)
        output2 = self.pool(self.final2(x2_2))
        output3 = self.pool(self.pool(self.final3(x1_3)))
        output4 = self.pool(self.pool(self.pool(self.final4(x0_4))))
