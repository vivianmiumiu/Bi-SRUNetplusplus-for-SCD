...
from models.BiSRNet_Unetplusplusimport BiSRNet as Net


NET_NAME = 'BiSRUNetplusplus'
DATA_NAME = 'BiSRUNetplusplus'
###############################################    
...
def train(train_loader, net, criterion, optimizer, scheduler, val_loader):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            train_bn_loss.update(train_bn_loss.avg)
            ...
            if (i + 1) % args['print_freq'] == 0:
                ...
                '[epoch %d] [iter %d / %d %.1fs] [lr %f] [train seg_loss %.4f bn_loss %.4f acc %.2f]' % (
                    curr_epoch, i + 1, len(train_loader), curr_time, optimizer.param_groups[0]['lr'],
                    train_seg_loss.val, train_bn_loss.val, acc_meter.val*100) #sc_loss %.4f, train_sc_loss.val,
        ...
        print('epoch %d Total time: %.1fs Best rec: Train acc %.2f, Val Fscd %.2f acc %.2f loss %.4f train_ loss %.4f' %(curr_epoch, time.time()-begin_time, bestaccT*100, bestFscdV*100, bestaccV*100, bestloss ,loss))
        curr_epoch += 1
        #scheduler.step()
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        if curr_epoch%args['predict_step']==0 and vi==0:
            ...
            io.imsave(os.path.join(args['pred_dir'], NET_NAME+'_A.tif'), pred_A_color)
            io.imsave(os.path.join(args['pred_dir'], NET_NAME+'_B.tif'), pred_B_color)
